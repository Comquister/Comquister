...
var Nodes = {
  ...
  // FPS Control
  fps: 60,
  fpsInterval: 1000 / 60, // 33.33ms para 30 FPS
  lastFrameTime: 0,
  ...
  init: function() {
    ...
    // Try to load logo.png, fallback to default pattern
    this.loadData('./logo.png');
  },

  createDefaultPattern: function() {
    ...
    var centerX = this.bgCanvas.width / 2;
    var centerY = this.bgCanvas.height / 2;
    
    // Draw some circles and shapes
    for (let i = 0; i < 5; i++) {
      this.bgContext.beginPath();
      this.bgContext.arc(centerX + Math.cos(i * Math.PI * 2 / 5) * 100, 
                        centerY + Math.sin(i * Math.PI * 2 / 5) * 100, 
                        30, 0, Math.PI * 2);
      this.bgContext.fill();
    }
    ...
    // Central circle
    this.bgContext.beginPath();
    this.bgContext.arc(centerX, centerY, 50, 0, Math.PI * 2);
    this.bgContext.fill();
  ...
  preparePoints: function() {
    ...
    for( var i = 0; i < this.canvas.height; i += this.density ) {
      for ( var j = 0; j < this.canvas.width; j += this.density ) {
        var pixelPosition = ( j + i * this.bgContextPixelData.width ) * 4;

        // Don't use whiteish pixels or transparent
  ...
  updatePoints: function() {
    for (var i = 0; i < this.points.length; i++ ){
      ...
      var theta = Math.atan2( currentPoint.y - this.mouse.y, currentPoint.x - this.mouse.x);
      var distance;
  ...
  drawLines: function() {
    for ( var i = 0; i < this.points.length; i++ ) {
      ...
      for ( var j = 0; j < this.points.length; j++ ){
        ...
        var distance = Math.sqrt((otherPoint.x - currentPoint.x) * (otherPoint.x - currentPoint.x) +
  ...
  draw: function() {
    ...
    // Se passou tempo suficiente para o próximo frame (30 FPS)
    if (elapsed > this.fpsInterval) {
      // Ajusta lastFrameTime, mas mantém o excesso para suavidade
  ...
  loadData: function( data ) {
    ...
    // If image fails to load, use default pattern
    this.bgImage.onerror = function() {
      self.createDefaultPattern();
    }
  },
